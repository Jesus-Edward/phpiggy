...
class ReceiptService
{
    ...
    public function validateFile(?array $file)
    {
        ...
        if ($file['size'] > $maxFileSizeMB) {
            throw new ValidationException([
                'receipt' => ['File size exceeded max size']
            ]);
        }
    ...
    public function upload(array $file, int $transaction)
    {
        ...
        if (!move_uploaded_file($file['tmp_name'], $uploadPath)) {
            throw new ValidationException([
                'receipt' => ['Failed to  upload a file']
            ]);
        }

        $this->db->query(
            "INSERT INTO receipts(
                transaction_id, original_filename, storage_filename, file_type
            ) VALUE(:transaction_id, :original_filename, :storage_filename, :file_type)",
            [
